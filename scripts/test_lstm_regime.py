...
from models.lstm.lstm_regime import train_lstm_regime_model
...
def evaluate_results(results_df):
    """
    Perform full evaluation of LSTM test predictions for regime classification.
    """
    ...
    # Handle binary confusion matrix unpack safely
    if cm.shape == (2, 2):
...
""")
        ...
        print("⚠️ Confusion matrix is not 2x2. Possibly only one class in test set.")
...
def main():
    print("🚀 Starting Full LSTM Regime Evaluation...\n")

    # Train regime LSTM
    model, history, results_df = train_lstm_regime_model(
        symbol="BTCUSD",
        resolution="1h",
        ...
        epochs=10,
        batch_size=64,
        model_path="lstm_regime_model_test.h5"
    )
