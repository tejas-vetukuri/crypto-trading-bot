...
from models.alternate.gnn import (
    train_gnn_model,
    get_default_start_date,
    build_gnn_save_paths,
)
...
def build_threshold_summary(y_true: np.ndarray, probs: np.ndarray, threshold: float) -> dict:
    ...
    result["random_baseline_accuracy"] = random_baseline_with_coverage(y_true, coverage=coverage, seed=42)
    result["majority_baseline_accuracy"] = majority_baseline_with_coverage(y_true, coverage=coverage)
...
def build_gnn_threshold_sweep(
    ...
    for threshold in thresholds:
        summary = build_threshold_summary(y_true=y_true, probs=probs, threshold=float(threshold))
...
def main():
    ...
    save_paths = build_gnn_save_paths(symbol, resolution)
    ...
    if RUN_TRAIN:
        model, artifacts, probs_df = train_gnn_model(
            symbol=symbol,
        ...
        print("📂 Skipping training. Loading saved predictions...")
        probs_df = pd.read_csv(save_paths["preds_csv_path"])
    ...
    summary = build_threshold_summary(y_true=y_true, probs=probs, threshold=chosen_threshold)
    ...
    sweep_df = build_gnn_threshold_sweep(y_true=y_true, probs=probs, thresholds=thresholds)
    print(sweep_df)
    ...
    print("\n✅ Full GNN evaluation completed successfully")
