...
from models.alternate.lstm_returns import (
    train_lstm_returns_model,
    get_default_start_date,
    build_lstm_returns_save_paths,
)
...
def build_lstm_returns_threshold_sweep(
    y_true: np.ndarray,
    probs: np.ndarray,
...
def main():
    ...
    save_paths = build_lstm_returns_save_paths(symbol, resolution)

    chosen_threshold = 0.57
    thresholds = (0.50, 0.53, 0.55, 0.57)

    RUN_TRAIN = False

    print("🚀 Starting LSTM Returns Evaluation...\n")
    ...
    if RUN_TRAIN:
        model, history, probs_df, metrics_df, scaler = train_lstm_returns_model(
            symbol=symbol,
    ...
    sweep_df = build_lstm_returns_threshold_sweep(
        y_true=y_true,
        probs=probs,
        thresholds=thresholds,
    )
    ...
    print("\n✅ Full LSTM Returns evaluation completed successfully")
